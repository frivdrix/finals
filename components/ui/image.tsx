...
import { type FittingType, getPlaceholder, sdk } from "@/utils/image-utils" // Assuming these are declared in a separate file
import "./image.css"
...
const getImageData = (url: string): ImageData | undefined => {
  ...
  if (url.startsWith(wixImagePrefix)) {
    ...
    const width = Number.parseInt(params.get("originWidth") || "0", 10)
    const height = Number.parseInt(params.get("originHeight") || "0", 10)
    ...
    if (urlObj.searchParams.get("originWidth") && urlObj.searchParams.get("originHeight")) {
      ...
      const width = Number.parseInt(urlObj.searchParams.get("originWidth") || "0", 10)
      const height = Number.parseInt(urlObj.searchParams.get("originHeight") || "0", 10)
      return { id: uri, width, height }
    }
  }
}
...
const WixImage = forwardRef<HTMLImageElement, WixImageProps>(
  ({ data, fittingType = "fill", ...imgProps }, parentRef) => {
    ...
    return <img ref={ref} {...imgProps} src={src || "/placeholder.svg"} />
  },
)
